...
public class StudentSignUp extends JFrame implements Serializable{
...
/*
	method to build the center panel of the content oane.
*/
...
/*
	Action Listener for all the buttons used in he GUI.
		...
		public void actionPerformed(ActionEvent event){
			...
			if(event.getActionCommand().equals("Submit")){
				if(!check.isSelected()){
					...
					String name, rollNo, session, department, email, password, confirmPassword, authority;
					name=textField[0].getText();
					session=textField[3].getText();
					department=textField[4].getText();
					email=textField[5].getText();
					rollNo=textField[2].getText();
					authority="Student";
					password=passwordField[0].getText();
					confirmPassword=passwordField[1].getText();
					if((name.equals("") || name==null) || (session.equals("") || session==null) || (department.equals("") || department==null) || (email.equals("") || email==null) || (rollNo.equals("")||rollNo==null) || (password.equals("") || password==null) || (confirmPassword.equals("") || confirmPassword==null)){
					...
					try{
						file=new File("Database/Student/"+name+".dat");
						if(file.exists()){
							JOptionPane.showMessageDialog(null,"User Name Already Exists.");
						}
						else{
						fileOutput=new FileOutputStream("Database/Student/"+name+".dat");
						objectOutput=new ObjectOutputStream(fileOutput);
						objectOutput.writeObject(new LoginDatabase(name, password, email, session, department, rollNo, authority));
						objectOutput.close();
...
/*
	method to build the south panel of the content pane.
*/
